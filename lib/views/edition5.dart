...
class EditionCinq extends StatefulWidget {
  ...
  @override
  _EditionCinqState createState() => _EditionCinqState();
}

class _EditionCinqState extends State<EditionCinq> {
  List<int> ids = [38, 39, 40, 41]; //Palmares images
  List<int> idsNom = [63, 64, 66, 67]; //Palmares Noms
  int num = 5;
...
//image jury
  Future<String> _fetchImage() async {
    try {
      String imageUrl = await APIManager.fetchImage(num, 42);
      return imageUrl;
    } catch (error) {
  ...
  Future<List<String>> _fetchImageThree() async {
    try {
      List<String> imageUrl =
          await APIManager.fetchImageUrl([43, 44, 45, 46, 47], num);
      return imageUrl;
    } catch (error) {
      print('Erreur lors de la récupération de l\'URL de l\'image: $error');
      return [];
  ...
  Future<List<Partcipation>> _fetchData() async {
    ...
    try {
      List<String> awards = await APIManager.fetchParticipationAwards(
          [77, 78, 79, 80, 81], num); //Participation
      List<String> films = await APIManager.fetchEditionFilms(
          [77, 78, 79, 80, 81], num); //Participaton
  ...
  Future<void> _fetchPCourtParticipants() async {
    try {
      List<Participant> participants =
          await APIManager.fetchParticipants([77, 78, 79, 80, 81]); //paticipant
      if (participants != null && participants.isNotEmpty) {
        setState(() {
