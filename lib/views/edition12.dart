...
class EditionDouze extends StatefulWidget {
  ...
  @override
  _EditionDouzeState createState() => _EditionDouzeState();
}

class _EditionDouzeState extends State<EditionDouze> {
  List<int> ids = [125, 126, 127, 128]; //Palmares images
  List<int> idsNom = [219, 220, 221, 222]; //Palmares Noms
  int num = 12;
...
//image jury
  Future<String> _fetchImage() async {
    try {
      String imageUrl = await APIManager.fetchImage(num, 129);
      return imageUrl;
    } catch (error) {
  ...
  Future<List<String>> _fetchImageThree() async {
    try {
      List<String> imageUrl =
          await APIManager.fetchImageUrl([130, 131, 132, 133, 134], num);
      return imageUrl;
    } catch (error) {
      print('Erreur lors de la récupération de l\'URL de l\'image: $error');
      return [];
  ...
  Future<List<Partcipation>> _fetchData() async {
    ...
    try {
      List<String> awards = await APIManager.fetchParticipationAwards(
          [232, 233, 234, 235, 236], num); //Participation
      List<String> films = await APIManager.fetchEditionFilms(
          [232, 233, 234, 235, 236], num); //Participaton
...
//hOMMAGE
  Future<void> _fetchPCourtParticipants() async {
    try {
      List<Participant> participants = await APIManager.fetchParticipants(
          [232, 233, 234, 235, 236]); //paticipant
      if (participants != null && participants.isNotEmpty) {
        setState(() {
...
//MasterClass
  ...
  Future<List<String>> _fetchImageMaster() async {
    try {
      List<String> imageUrl =
          await APIManager.fetchImageUrl([135, 136, 137, 138], num);
      return imageUrl;
    } catch (error) {
      print('Erreur lors de la récupération de l\'URL de l\'image: $error');
      return [];
...
//MasterClass
  ...
  Future<List<Partcipation>> _fetchDataMaster() async {
    ...
    try {
      List<String> awards = await APIManager.fetchParticipationAwards(
          [237, 238, 239, 240], num); //Participation
      List<String> films = await APIManager.fetchEditionFilms(
          [237, 238, 239, 240], num); //Participaton
      for (int i = 0; i < awards.length; i++) {
        Partcipation participation = Partcipation.empty();
        participation.setPrix(awards[i]);
        participation.setFilm(films[i]);
  ...
  Future<void> _fetchMaster() async {
    try {
      List<Participant> participants =
          await APIManager.fetchParticipants([237, 238, 239, 240]); //paticipant
      if (participants != null && participants.isNotEmpty) {
        setState(() {
